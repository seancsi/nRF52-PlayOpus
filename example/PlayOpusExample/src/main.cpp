...
void loadBuffer(uint16_t * dest, const uint16_t * src, size_t samples);

extern const uint16_t data[];

uint32_t offset = 0;

uint16_t bufA[BUFFER_LENGTH * 2];
uint16_t bufB[BUFFER_LENGTH * 2];
...
void loadBuffer(uint16_t * dest, const uint16_t * src, size_t samples) {
  ...
  for (i = 0; i < samples; i++) {
    if ( dataFile.available() )
      dest[i] = ( dataFile.read() | (dataFile.read() << 8) );
    else
      nrfx_i2s_stop();
  }
}
